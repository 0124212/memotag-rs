...
/// A CalDAV VTODO item as represented in memory.
#[derive(Debug, Clone)]
pub struct VTodo {
    pub uid: String,
    pub summary: String,
...
impl CalDavClient {
    ...
    pub async fn ensure_collection(&self) -> Result<()> {
        // First try PROPFIND to see if it exists
        ...
        if resp.status().is_success() || resp.status().as_u16() == 201 {
            ...
            // Some servers return 405 for existing collection - that's fine
    ...
    /// List all VTODO items in the collection.
    pub async fn list_vtodos(&self) -> Result<Vec<CalDavResource>> {
        let resp = self.client
            .request(reqwest::Method::from_bytes(b"REPORT").unwrap(), &self.collection_url)
            .header("Authorization", &self.auth)
            .header("Depth", "1")
            .body(VTODO_REPORT.to_string())
            .send()
            .await
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            anyhow::bail!("list vtodos failed {}: {}", status, &body[..body.len().min(300)]);
        }
    ...
    /// Get a single VTODO by href.
    pub async fn get_vtodo(&self, href: &str) -> Result<Option<CalDavResource>> {
        ...
        if !status.is_success() {
            anyhow::bail!("get vtodo failed {}: {}", status, &body[..body.len().min(200)]);
        }
    ...
    /// Create or update a VTODO item.
    pub async fn put_vtodo(&self, href: &str, vtodo: &VTodo, if_match: Option<&str>) -> Result<String> {
        let url = format!("{}/{}", self.base_url.trim_end_matches('/'), href.trim_start_matches('/'));
        ...
        let resp = req.send().await.context("PUT VTODO")?;
        let status = resp.status();
        ...
        if !status.is_success() && status.as_u16() != 201 && status.as_u16() != 204 {
            let body = resp.text().await.unwrap_or_default();
            anyhow::bail!("put vtodo failed {}: {}", status, &body[..body.len().min(200)]);
        }
    ...
    /// Delete a VTODO item.
    pub async fn delete_vtodo(&self, href: &str) -> Result<()> {
        let url = format!("{}/{}", self.base_url.trim_end_matches('/'), href.trim_start_matches('/'));
        let resp = self.client
            .delete(&url)
            .header("Authorization", &self.auth)
            .send()
            .await
            .context("DELETE VTODO")?;

        let status = resp.status();
        if !status.is_success() && status.as_u16() != 404 {
            let body = resp.text().await.unwrap_or_default();
            warn!("delete vtodo failed {}: {}", status, &body[..body.len().min(200)]);
        }
        Ok(())
    }
}
...
pub fn build_vtodo_ical(vtodo: &VTodo) -> String {
    ...
    let description = if let Some(ref desc) = vtodo.description {
...
pub fn parse_vtodo(data: &str) -> Option<VTodo> {
    ...
    let re_description = Regex::new(r"DESCRIPTION:(.+)").ok()?;
    ...
    // Override: if COMPLETED property exists, treat as completed
    ...
    let description = re_description.captures(data)
        .and_then(|c| c.get(1))
        .map(|m| {
            m.as_str()
                .replace("\\n", "\n")
...
/// Build a CalDAV REPORT body to fetch all VTODO items.
const VTODO_REPORT: &str = r#"<?xml version="1.0" encoding="utf-8"?>
...
<c:mkcalendar xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav">
  <d:set>
    <d:prop>
      ...
      <c:calendar-description>Synced tasks from memos</c:calendar-description>
...
mod tests {
    ...
    fn test_build_vtodo_ical() {
        ...
        assert!(ical.contains("DUE;VALUE=DATE:20260415"));
    ...
    fn test_parse_vtodo() {
        ...
        assert_eq!(parsed.uid, "test-456");
        assert_eq!(parsed.summary, "File taxes");
        assert_eq!(parsed.status, VTodoStatus::Completed);
